...
void ClearScene::Initialize(SceneManager* sceneManager) {
	...
	ClearWT_.Initialize();
	ClearWT_.translation_ = {0.0f, 50.0f, 0.0f};
	ClearWT_.scale_ = {10.0f, 10.0f, 10.0f};
	ClearWT_.rotation_.y = -1.5f;
...
void ClearScene::Update() {
	...
	ClearWT_.translation_.y = 50.0f + bounce;
	ClearWT_.UpdateMatrix();
...
void ClearScene::Draw() {
	...
	Model::PreDraw(Model::CullingMode::kNone, Model::BlendMode::kNormal, Model::DepthTestMode::kOn);
	if (ClearModel_) {
		ClearModel_->Draw(ClearWT_, *camera_, &objectColor_);
	}
	Model::PostDraw();
